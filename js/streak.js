...
function getCalendarHTML(date, streakDays, goalNum) {
  ...
  for (let i = 0; i < dayGrid.length; i++) {
    ...
    outputHTML += `<div class="calendar-day day-${dayAdd[1]}" title="${dayAdd[0]} xp" xp="${dayAdd[0]}">${dayAdd[2]}</div>`;
  }
  outputHTML += `</div></div>`;
  return outputHTML;
}
$(document).on("click", "body:not([header-collapsed]) [streak='calendar']", (e) => {
  calJSON = JSON.parse(localStorage.getItem("userData"));
...
$(document.body).on("click", ".calendar-button", (e) => {
  ...
  cal.html(getCalendarHTML(new Date(cal.attr("date")), calJSON.xphistory, calJSON.goal));
});
